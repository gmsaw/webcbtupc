    ...
    <div x-show="showFinishModal" style="display: none;" class="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
        <div class="bg-white rounded-[2rem] p-6 sm:p-8 max-w-md w-full text-center shadow-2xl transform transition-all border-t-8 border-blue-600">
            ...
            <div class="flex gap-3 sm:gap-4">
                <button @click="showFinishModal = false" class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 sm:py-3.5 rounded-xl text-sm sm:text-base transition-all active:scale-95 border border-slate-200">
                    Batal
                </button>
                <button @click="autoSubmit()" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 sm:py-3.5 rounded-xl text-sm sm:text-base transition-all active:scale-95 shadow-lg shadow-blue-600/30">
                    Kumpulkan
                </button>
            </div>
        </div>
    ...
    <header class="bg-gradient-to-r from-blue-700 to-indigo-800 text-white shadow-lg sticky top-0 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex justify-between items-center">
            ...
            <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                ...
                <div class="hidden sm:flex bg-white/10 p-2.5 rounded-xl backdrop-blur-sm border border-white/10 shrink-0">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
                </div>
                <div class="truncate pr-2">
                    <h1 class="font-bold text-sm sm:text-lg leading-tight truncate">{{ $competition->nama_lomba }}</h1>
                    <p class="text-[10px] sm:text-xs text-blue-200 font-medium truncate">{{ Auth::user()->name }}</p>
                </div>
            </div>

            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                <div class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-colors"
                    ...
                    <svg x-show="syncStatus === 'idle'" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                    <svg x-show="syncStatus === 'saving'" class="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    <svg x-show="syncStatus === 'saved'" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                ...
                <div class="bg-red-500/90 backdrop-blur-md px-2.5 sm:px-5 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl border border-red-400 shadow-inner flex items-center gap-2 transition-colors" :class="{'animate-pulse bg-red-600 border-red-300': timeRemaining < 300}">
                    ...
                    <div class="text-right">
                        ...
                        <span class="font-mono font-black text-sm sm:text-xl leading-none tracking-tight" x-text="formatTime()">00:00:00</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="bg-slate-50/80 border-b border-slate-100 px-4 sm:px-8 py-3.5 sm:py-5 flex justify-between items-center sticky top-0 z-10 backdrop-blur-sm">
                    ...
                    <label class="flex items-center gap-1.5 sm:gap-2.5 cursor-pointer bg-white border border-slate-200 hover:border-yellow-400 hover:bg-yellow-50 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl transition-all shadow-sm select-none group" :class="{'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-400/20': doubtful[activeQuestionIndex]}">
                        <div class="relative flex items-center justify-center">
                            ...
                            <div class="w-4 h-4 sm:w-5 sm:h-5 border-2 rounded transition-colors" :class="doubtful[activeQuestionIndex] ? 'border-yellow-500 bg-yellow-500' : 'border-slate-300 group-hover:border-yellow-400'">
                        ...
                        <span class="font-bold text-xs sm:text-sm transition-colors" :class="doubtful[activeQuestionIndex] ? 'text-yellow-700' : 'text-slate-600 group-hover:text-yellow-600'">Ragu-ragu</span>
                    </label>
                </div>
                    ...
                    <div class="space-y-2.5 sm:space-y-4">
                        <template x-for="(opsi, huruf) in questions[activeQuestionIndex].options" :key="huruf">
                            <label class="flex items-start gap-3 sm:gap-4 p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border-2 cursor-pointer transition-all duration-200 group relative overflow-hidden active:scale-[0.99]"
                                ...
                                <div class="flex-1 pt-1 sm:pt-1.5 text-slate-700 font-medium text-sm sm:text-base tex2jax_process z-10" x-html="opsi"></div>
        ...
        <div class="fixed inset-y-0 right-0 z-50 w-[85%] sm:w-80 bg-white shadow-2xl overflow-y-auto transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 lg:w-[19rem] lg:shadow-none lg:bg-transparent lg:z-0 lg:overflow-visible no-scrollbar flex flex-col"
            ...
            <div class="bg-white lg:rounded-3xl lg:shadow-xl lg:shadow-blue-900/5 lg:border lg:border-slate-100 flex-1 lg:flex-none flex flex-col lg:sticky lg:top-24 h-full lg:h-[calc(100vh-8rem)]">
                ...
                <div class="p-4 sm:p-5 flex-1 overflow-y-auto no-scrollbar pb-6">
                    ...
                    <div class="space-y-2.5 text-[11px] sm:text-xs font-bold text-slate-600 bg-slate-50 p-4 rounded-xl border border-slate-100">
                        <div class="flex items-center gap-2.5">
                            <div class="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-emerald-500 shadow-sm shadow-emerald-500/30"></div> <span>Sudah Dijawab</span>
                        </div>
                        <div class="flex items-center gap-2.5">
                            <div class="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-yellow-100 border-2 border-yellow-400"></div> <span>Ragu - ragu</span>
                        </div>
                        <div class="flex items-center gap-2.5">
                            <div class="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-white border-2 border-slate-200"></div> <span>Belum Dijawab</span>
                        </div>
                    </div>
                </div>

                <div class="p-4 sm:p-5 border-t border-slate-100 bg-white lg:rounded-b-3xl shrink-0 mt-auto">
                    <button @click="confirmFinish()" class="w-full bg-rose-50 text-rose-600 border-2 border-rose-200 hover:bg-rose-600 hover:text-white hover:border-rose-600 font-bold py-3 sm:py-3.5 rounded-xl text-sm transition-all shadow-sm flex items-center justify-center gap-2 active:scale-95">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                        Akhiri Ujian
                    </button>
                </div>
