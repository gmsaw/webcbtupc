<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-bold text-2xl text-gray-800 leading-tight">{{ __('Buat Kompetisi Baru') }}</h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-8">
                ...
                <form action="{{ route('admin.kompetisi.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="competitionForm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <x-text-input id="nama_lomba" name="nama_lomba" type="text" class="mt-1 block w-full rounded-xl @error('nama_lomba') border-red-500 @enderror" required value="{{ old('nama_lomba') }}" placeholder="Cth: Olimpiade Fisika SMA" autofocus />
                                @error('nama_lomba') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <x-input-label for="deskripsi" value="Deskripsi" />
                                <textarea id="deskripsi" name="deskripsi" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm @error('deskripsi') border-red-500 @enderror" placeholder="Deskripsi singkat kompetisi...">{{ old('deskripsi') }}</textarea>
                                @error('deskripsi') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            {{-- Sistem Biaya --}}
                            @php $oldWaves = old('waves', [['nama_gelombang' => '', 'start_date' => '', 'end_date' => '', 'biaya' => '']]); @endphp
                            <div x-data="{ isUsingWaves: {{ old('is_using_waves') ? 'true' : 'false' }}, waves: {{ json_encode($oldWaves) }} }" class="p-5 bg-slate-50 rounded-2xl border border-slate-200">
                                ...
                                <div x-show="!isUsingWaves" x-transition>
                                    ...
                                    <x-text-input name="harga_pendaftaran" type="number" class="mt-1 block w-full rounded-xl @error('harga_pendaftaran') border-red-500 @enderror" min="0" value="{{ old('harga_pendaftaran', 0) }}" placeholder="Ketik 0 jika gratis" />
                        ...
                        <div class="space-y-6">
                            <div class="grid grid-cols-2 gap-4 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-50">
                                <div>
                                    ...
                                    <x-text-input id="tanggal_mulai" name="tanggal_mulai" type="date" class="mt-1 block w-full rounded-xl text-sm @error('tanggal_mulai') border-red-500 @enderror" required value="{{ old('tanggal_mulai', \Carbon\Carbon::today()->format('Y-m-d')) }}" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="tanggal_selesai" name="tanggal_selesai" type="date" class="mt-1 block w-full rounded-xl text-sm @error('tanggal_selesai') border-red-500 @enderror" required value="{{ old('tanggal_selesai', \Carbon\Carbon::today()->addDays(30)->format('Y-m-d')) }}" />
                            ...
                            <div class="grid grid-cols-3 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                                ...
                                <div>
                                    ...
                                    <x-text-input name="nilai_benar" type="number" step="0.1" class="mt-1 block w-full text-sm rounded-xl" value="{{ old('nilai_benar', 4) }}" required />
                                </div>
                                <div>
                                    <x-input-label value="Poin Salah" />
                                    <x-text-input name="nilai_salah" type="number" step="0.1" class="mt-1 block w-full text-sm rounded-xl" value="{{ old('nilai_salah', -1) }}" required />
                                </div>
                                <div>
                                    <x-input-label value="Poin Kosong" />
                                    <x-text-input name="nilai_kosong" type="number" step="0.1" class="mt-1 block w-full text-sm rounded-xl" value="{{ old('nilai_kosong', 0) }}" required />
                                </div>
                            </div>
                    ...
                    <div class="flex items-center gap-3 pt-6 border-t border-gray-100">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 w-5 h-5 cursor-pointer">
                        <label for="is_active" class="font-medium text-gray-900 text-lg cursor-pointer">Buka Pendaftaran Sekarang (Status Aktif)</label>
                    </div>
